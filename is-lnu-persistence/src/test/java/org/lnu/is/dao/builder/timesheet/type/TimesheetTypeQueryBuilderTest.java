...
public class TimesheetTypeQueryBuilderTest {
	...
	public void testBuildWithParameters() throws Exception {
		...
		String expectedQuery = "SELECT e FROM TimeSheetType e WHERE ( e.name LIKE CONCAT('%',:name,'%') AND e.abbrName LIKE CONCAT('%',:abbrName,'%') ) AND e.status=:status AND e.crtUserGroup IN (:userGroups) ";
		MultiplePagedSearch<TimeSheetType> pagedSearch = new MultiplePagedSearch<>();
		pagedSearch.setEntity(context);
	...
	public void testBuildWithParametersAndWithourDefaultConstraints() throws Exception {
		...
		String expectedQuery = "SELECT e FROM TimeSheetType e WHERE ( e.name LIKE CONCAT('%',:name,'%') AND e.abbrName LIKE CONCAT('%',:abbrName,'%') ) ";
		MultiplePagedSearch<TimeSheetType> pagedSearch = new MultiplePagedSearch<>();
		pagedSearch.setEntity(context);
