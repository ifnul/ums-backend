...
public class SubjectTypeQueryBuilderTest {
	...
	public void testBuildWithParameters() throws Exception {
		...
		String expectedQuery = "SELECT e FROM SubjectType e WHERE ( e.name LIKE CONCAT('%',:name,'%') AND e.abbrName LIKE CONCAT('%',:abbrName,'%') ) AND e.status=:status AND e.crtUserGroup IN (:userGroups) ";
		MultiplePagedSearch<SubjectType> pagedSearch = new MultiplePagedSearch<>();
		pagedSearch.setEntity(context);
	...
	public void testBuildWithParametersWithDisabledDefaultConstraints() throws Exception {
		...
		String expectedQuery = "SELECT e FROM SubjectType e WHERE ( e.name LIKE CONCAT('%',:name,'%') AND e.abbrName LIKE CONCAT('%',:abbrName,'%') ) ";
		MultiplePagedSearch<SubjectType> pagedSearch = new MultiplePagedSearch<>();
		pagedSearch.setEntity(context);
