...
public class DefaultEmailSenderAdapterTest {
	...
	@Test
	public void testSendWithDisabled() throws Exception {
		// Given
		...
		// Then
		verify(mimeMessage, times(0)).setFrom(any(InternetAddress.class));
		verify(mimeMessage, times(0)).setRecipients(eq(Message.RecipientType.TO), any(InternetAddress[].class));
		verify(mimeMessage, times(0)).setRecipients(eq(Message.RecipientType.CC), any(InternetAddress[].class));
		verify(mimeMessage, times(0)).setSubject(topic, "UTF-8");
