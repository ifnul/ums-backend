...
public class SpecOfferWaveControllerTest extends AbstractControllerTest {
	...
	public void testCreateSpecOfferWave() throws Exception {
		...
		when(facade.createResource(any(SpecOfferWaveResource.class))).thenReturn(specOfferSubjectResource);
		
    	// Then
		mockMvc.perform(post("/specoffers/{0}/waves", specOfferId)
	...
	public void testUpdateSpecOfferWave() throws Exception {
    	...
    	// Then
		mockMvc.perform(put("/specoffers/{0}/waves/{id}", specOfferId, id)
				.contentType(MediaType.APPLICATION_JSON)
				.content(request))
	...
	public void testGetSpecOfferWaves() throws Exception {
		...
		PagedRequest<SpecOfferWaveResource> pagedRequest = new PagedRequest<SpecOfferWaveResource>(resource, offset, limit, Collections.<OrderBy>emptyList());
    ...
    @Test
    public void testGetSpecOfferWavesGeneral() throws Exception {
    	// Given
    	Long id = 1L;
    	Long specOfferId = 2L;
    	...
    	int limit = 25;
    	Integer offset = 10;
    	String uri = MessageFormat.format("/persons/waves", specOfferId);
    	List<SpecOfferWaveResource> entities = Arrays.asList(specOfferSubjectResource);
    	PagedResultResource<SpecOfferWaveResource> expectedResource = new PagedResultResource<>();
    	expectedResource.setCount(count);
    	expectedResource.setLimit(limit);
    	...
    	// Then
    	mockMvc.perform(get("/specoffers/waves", specOfferId)
    			.param("offset", String.valueOf(offset))
    			.param("limit", String.valueOf(limit)))
    			.andExpect(status().isOk())
    			.andExpect(content().string(response));
	...
	public void testGetResourceWithAccessDeniedException() throws Exception {
		...
		// Then
		mockMvc.perform(get("/specoffers/{specOfferId}/waves/{id}", specOfferId, id));
