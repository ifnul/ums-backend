...
import org.lnu.is.edbo.guides_min.DLanguages;
import org.lnu.is.edbo.guides_min.DUniversities;
...
class Edbo {
	...
	class WorkAddSpecOffer implements Callable<Void>
	{
		...
		public Void call() throws Exception {
			try {
				...
				Long specofferId = CallService.Instance().AddSpecoffer(spec);
				...
				for (DUniversityFacultetSpecialitiesSubjects2 subject : subjects)
					CallService.Instance().AddSpecofferSubject(specofferId, subject);
				...
				for (DUniversityFacultetSpecialitiesQuotas quota : quotas)
					CallService.Instance().AddSpecofferQuota(specofferId, quota);
    ...
    private void Init() throws RemoteException {
        ...
        Language = guides.languagesGet(ConnectionEDBO.SessionGuid).getDLanguages().get(0);
        try {
    ...
    public void GetAllRequest() throws Exception {
		...
		for (DUniversityFacultetSpecialities spec : soecialities) {
			...
			for (DUniversityFacultetsRequests2 zayava : reqst) {
				++i;
				tasks.add(new WorkAddRequest(zayava, i));
			}
